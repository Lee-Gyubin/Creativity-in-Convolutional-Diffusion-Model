...
import random
from reward import compute_l1_distance, create_checkerboard
...
class FKD:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.potential_type = PotentialType(potential_type) #TODO
        self.lmbda = lmbda
        ...
        self.latent_to_decode_fn = latent_to_decode_fn #만약, latent diffusion이 아니라면 그냥 identity function인 듯
    ...
    def resample(
        ...
        if int(sampling_idx) not in resampling_interval:
            return latents, x0_preds #HERE TODO
...
if __name__ == "__main__":
    ...
    # 1x1 pixel images
    num_particles = 8
    x0s = torch.rand(num_particles, 1, 16, 16)
    ...
    fkds = FKD(
        ...
        time_steps=100,
        reward_fn=lambda x: reward_function(x,checkerboard),
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu"),
    )
    ...
    for i in range(num_particles):
        axs[0, i].imshow(x0s[i].squeeze().detach().numpy(), cmap='gray', vmin=0, vmax=1)
        axs[1, i].imshow(
            resampled_images[i].squeeze().detach().numpy(), cmap='gray', vmin=0, vmax=1
        )
